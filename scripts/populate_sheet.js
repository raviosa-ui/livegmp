...
// ---------------- CREDENTIALS SETUP (✅ MODIFIED FOR RAW JSON FIX) ----------------
...
// ✅ NEW: Read the unencoded JSON directly from the new environment variable
const RAW_SA_JSON_FROM_ENV = process.env.GOOGLE_APPLICATION_CREDENTIALS_RAW || "";

let CLIENT_EMAIL = "";
let PRIVATE_KEY = "";
let auth = null; 

if (RAW_SA_JSON_FROM_ENV) {
  try {
    // Directly parse the RAW JSON text, skipping the problematic Base64 step
    const parsed = JSON.parse(RAW_SA_JSON_FROM_ENV);
    CLIENT_EMAIL = parsed.client_email;
    PRIVATE_KEY = parsed.private_key;
    console.log("Service Account key successfully parsed from RAW JSON.");
  } catch (e) {
    console.error("❌ CRITICAL: Failed to parse GOOGLE_SERVICE_ACCOUNT_RAW. Check your secret's JSON formatting.", e && e.message);
    process.exit(1);
  }
}

// ❌ REMOVED: All the old logic for checking RAW_B64, GOOGLE_SERVICE_ACCOUNT_JSON, etc.

if (!SHEET_ID || !CLIENT_EMAIL || !PRIVATE_KEY) {
  console.error("❌ Missing Google API secrets. Ensure GOOGLE_APPLICATION_CREDENTIALS_RAW and GOOGLE_SHEET_ID are set.");
  process.exit(1);
}
// ---------------- GOOGLE AUTH (✅ MODIFIED FOR DIRECT JWT CREATION) ----------------
// Create the JWT instance directly using the parsed key.
try {
    ...
    console.error("❌ Failed to create Google JWT instance:", e && e.message);
    process.exit(1);
}
